...
def generate_synthetic_real_estate_data(
    ...
    Economic structure:
    - Property values are driven by hedonic features and location quality.
    - Macroeconomic factors (rates, unemployment, inflation, GDP growth) affect prices.
    - Time trend and cyclical components are included.
    """
    ...
    # Structural price model in logs
    loc_quality = (
...
def evaluate_models(X: pd.DataFrame, y: pd.Series, random_state: int = 42) -> Tuple[pd.DataFrame, Dict[str, object], tuple]:
    ...
    result = pd.DataFrame(rows).sort_values("rmse_test").reset_index(drop=True)
    return result, fitted, (X_train, X_test, y_train, y_test)
...
def build_cashflows(assump: InvestmentAssumptions, price_growth_path: np.ndarray, rent_growth_path: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    annual_payment = _annual_mortgage_payment(debt, assump.annual_interest_rate, assump.loan_amort_years)
    ...
    for y in range(1, T + 1):
        ...
        interest = outstanding * assump.annual_interest_rate
    ...
    payback = np.argmax(np.cumsum(cf) > 0)
    payback_period = int(payback) if np.cumsum(cf).max() > 0 else np.nan
...
def monte_carlo_investment(
    ...
    for _ in range(n_sims):
        ...
        cfs = build_cashflows(assump, price_growth, rent_growth)
        cf = cfs["cashflows"]

        sim_irr = irr(cf)
        sim_npv = npv(assump.discount_rate, cf)
...
def run_sensitivity_grid(base: InvestmentAssumptions) -> pd.DataFrame:
    """One-way and two-way sensitivity analysis across key assumptions."""
    ...
    for ir in interest_grid:
        for pp in price_grid:
            for rent in rent_grid:
                ...
                cf_data = build_cashflows(a, price_path, rent_path)
                cf = cf_data["cashflows"]
                rows.append(
                    {
                        "interest_rate": ir,
                        "purchase_price": pp,
